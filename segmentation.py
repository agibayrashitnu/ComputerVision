...
def kmeans_fast(features, k, num_iters=100):
    ...
    for n in range(num_iters):
        ...
        features_tmp = np.tile(features, (k, 1))
        centers_tmp = np.repeat(centers, N, axis=0)
        assignments = np.argmin(np.sum((features_tmp - centers_tmp)**2, axis=1).reshape(k, N), axis=0)
        tempo = centers.copy()
        for j in range(k):
            centers[j] = np.mean(features[assignments == j], axis=0)
        if np.allclose(tempo, centers):
